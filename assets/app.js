...
function renderFiles(files){
  const rows = files.map(f => `<tr><td>${esc(f.name)}</td><td>${esc(f.mimeType)}</td><td>${f.supported ? 'YES' : 'NO'}</td><td><a href="${f.url}" target="_blank">เปิด</a></td></tr>`).join('');
  document.getElementById('resultTable').innerHTML = `<table><thead><tr><th>ชื่อไฟล์</th><th>MIME</th><th>รองรับ</th><th>ลิงก์</th></tr></thead><tbody>${rows || '<tr><td colspan="4">ไม่พบไฟล์</td></tr>'}</tbody></table>`;
}

function renderResults(results){
  const rows = results.map(r => `<tr><td>${esc(r.file || '')}</td><td>${esc(r.status || '')}</td><td>${esc(r.category || '')}</td><td>${r.mean ?? ''}</td><td>${r.sd ?? ''}</td><td>${r.outputSpreadsheetUrl ? `<a href="${r.outputSpreadsheetUrl}" target="_blank">Sheet</a>` : ''}</td><td>${r.pdfUrl ? `<a href="${r.pdfUrl}" target="_blank">PDF</a>` : ''}</td></tr>`).join('');
  document.getElementById('resultTable').innerHTML = `<table><thead><tr><th>ไฟล์</th><th>สถานะ</th><th>หมวด</th><th>X</th><th>SD</th><th>Excel/Sheet</th><th>PDF</th></tr></thead><tbody>${rows || '<tr><td colspan="7">ไม่มีผลลัพธ์</td></tr>'}</tbody></table>`;
}
