...
const PARSER_HEADER_ROLES = [
  // คอลัมน์สรุป/สถิติที่คำนวณไว้แล้ว ต้องไม่ถูกนับเป็นข้อคำถาม (เช่นไฟล์จริงมี ค่าเฉลี่ย/SD ท้ายตาราง)
  { role: 'summary', regex: /(ค่าเฉลี่ย|เฉลี่ยรวม|^\s*เฉลี่ย\s*$|average|^\s*mean\s*$|ส่วนเบี่ยงเบน|^\s*s\.?\s*d\.?\s*$|รวมคะแนน|ผลรวมคะแนน|^\s*ผลรวม\s*$|^\s*total\s*$|ตอบ\s*\/\s*ทั้งหมด|จำนวนที่ตอบ|^\s*ระดับ\s*$)/i },
...
/** ธีมข้อคิดเห็น (keyword-based) — ชุดเริ่มต้นตาม SKILL.md v5.1 ข้อ 9 */
const PARSER_COMMENT_THEMES = [
  { theme: 'การให้คำปรึกษา/การบริการ', regex: /(ให้คำปรึกษา|คำปรึกษา|ปรึกษา|บริการ|ช่วยเหลือ|ดูแล|แนะนำ)/ },
  { theme: 'การสื่อสาร/การรับฟัง', regex: /(สื่อสาร|รับฟัง|ฟัง|พูดคุย|ประชาสัมพันธ์|แจ้ง|ข้อมูลข่าวสาร|ชี้แจง|กระชั้น|เวลา|ตาราง)/ },
  { theme: 'ความใส่ใจ/ความเอาใจใส่', regex: /(ใส่ใจ|เอาใจใส่|เข้าใจ|เป็นกันเอง|อบอุ่น|กำลังใจ)/ },
  { theme: 'ความเป็นส่วนตัว/ความลับ', regex: /(ส่วนตัว|ความลับ|เป็นความลับ|ปกปิด|ข้อมูลส่วนบุคคล)/ },
  { theme: 'ความพร้อมของสถานที่/ระบบ', regex: /(สถานที่|ห้อง|อุปกรณ์|เครื่อง|สัญญาณ|ระบบ|อากาศ|ร้อน|เสียง|อาหาร|น้ำ|สวัสดิการ)/ },
  { theme: 'ข้อเสนอแนะอื่น ๆ', regex: /(ดีมาก|ดี|ชอบ|ประทับใจ|ขอบคุณ|เยี่ยม|สนุก|พอใจ|กิจกรรม|ฝึก|สอน|เนื้อหา)/ }
];
