/**
 * Parent Evaluation API extension.
 * Loaded after existing v6 API overrides by the deployment workflow.
 */
function apiGet_(p) {
  let result;
  try {
    const action = String((p && p.action) || 'health').toLowerCase();

    if (action === 'parenthealth') result = parentHealth();
    ...
    else if (action === 'parentdashboard') result = parentGetDashboard({
      academicYear: p.academicYear || '',
      activityId: p.activityId || ''
    });
    else if (action === 'parentindividual') result = parentGetIndividualReport({
      academicYear: p.academicYear || '',
  ...
  return jsonp_(result, p && p.callback);
}