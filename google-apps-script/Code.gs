...
function buildUploadPage_(){
  return '<!doctype html><html lang="th"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>RTAFNC Upload</title></head><body style="font-family:TH Sarabun PSK,Sarabun,Tahoma,sans-serif;background:#f4f7fb;padding:24px;font-size:20px"><div style="max-width:980px;margin:auto;background:white;border-radius:22px;padding:24px"><h1>RTAFNC Evaluation Analyzer v6</h1><p>Source-driven: ไม่เดาแยกชั้นปี, อ่านคำถามจริงทุกข้อ, สร้าง Excel รายละเอียดครบ และ PDF print-ready</p><form id="f"><input type="file" name="rawFile" accept=".xlsx,.xls,.csv,.tsv" required><select name="categoryOverride"><option value="">ให้ระบบจำแนกอัตโนมัติ</option><option>นภาภิบาล</option><option>คุณลักษณะทางทหาร</option><option>เดินทางไกล</option><option>ทหาร_1_4</option><option>จิตอาสา</option><option>เจตคติ</option><option>อาจารย์ที่ปรึกษา</option></select><p><button type="button" onclick="upOnly()">อัปโหลดอย่างเดียว</button><button type="button" onclick="upProcess()">อัปโหลด + ประมวลผล</button><button type="button" onclick="status()">สถานะ</button><button type="button" onclick="processQ()">ประมวลผลคิว</button><button type="button" style="background:#0b7a3b;color:#fff;border:0;border-radius:10px;padding:10px 16px;font-weight:700;cursor:pointer" onclick="upGolden()">อัปโหลด + สร้างรายงาน (Excel+PDF)</button></p></form><pre id="out">พร้อมใช้งาน</pre></div><script>function p(x){document.getElementById("out").textContent=JSON.stringify(x,null,2)}function err(e){p({error:String(e&&e.message?e.message:e)})}function upOnly(){google.script.run.withSuccessHandler(p).withFailureHandler(err).uploadOnly(document.getElementById("f"))}function upProcess(){google.script.run.withSuccessHandler(p).withFailureHandler(err).uploadAndProcess(document.getElementById("f"))}function status(){google.script.run.withSuccessHandler(p).withFailureHandler(err).getSystemStatus()}function processQ(){google.script.run.withSuccessHandler(p).withFailureHandler(err).processPendingRawFiles()}function upGolden(){document.getElementById("out").textContent="กำลังอัปโหลดและสร้างรายงาน (อาจใช้เวลาสักครู่)...";google.script.run.withSuccessHandler(goldenDone).withFailureHandler(err).uploadAndGolden(document.getElementById("f"))}function goldenDone(r){if(!r||!r.ok){p(r);return}window.__g=r;var out=document.getElementById("out");out.innerHTML="<b>สำเร็จ: "+r.type+"</b><br>ไฟล์: "+r.file+"<br><pre>"+JSON.stringify(r.summary,null,1)+"</pre><span id=dlbar></span>";var bar=document.getElementById("dlbar");if(r.xlsxBase64){var bx=document.createElement("button");bx.textContent="ดาวน์โหลด Excel";bx.style.cssText="background:#0b7a3b;color:#fff;border:0;border-radius:10px;padding:10px 16px;margin-right:8px;cursor:pointer";bx.onclick=function(){dl("xlsx")};bar.appendChild(bx)}if(r.pdfBase64){var bp=document.createElement("button");bp.textContent="ดาวน์โหลด PDF";bp.style.cssText="background:#991b1b;color:#fff;border:0;border-radius:10px;padding:10px 16px;margin-right:8px;cursor:pointer";bp.onclick=function(){dl("pdf")};bar.appendChild(bp)}if(r.outputUrl){var la=document.createElement("a");la.href=r.outputUrl;la.target="_blank";la.textContent="เปิด Google Sheet";bar.appendChild(la)}}function dl(k){var r=window.__g;var m=(k=="xlsx")?["xlsxBase64","application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","xlsx"]:["pdfBase64","application/pdf","pdf"];var s=atob(r[m[0]]);var a=new Uint8Array(s.length);for(var i=0;i<s.length;i++)a[i]=s.charCodeAt(i);var blob=new Blob([a],{type:m[1]});var u=URL.createObjectURL(blob);var el=document.createElement("a");el.href=u;el.download=(r.typeKey||"golden")+"."+m[2];document.body.appendChild(el);el.click();el.remove();setTimeout(function(){URL.revokeObjectURL(u)},5000)}status();<\/script></body></html>';
}
